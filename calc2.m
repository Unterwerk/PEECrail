...
offset = 1.5;       % Abstand beider Schienen [m]
l = 5.8;            % Länge der Schiene [m]
...
position = zeros(2 * n, 2); % Spalte 1: y-Koordinate, Spalte 2: z-Koordinate
ip = 1;
for i1 = 1:length(profile)
    for i2 = 1:profile(i1)
        ...
        position(n + ip, 1) = (i2 - 0.5) * grid + offset;
        position(n + ip, 2) = (i1 - 0.5) * grid;
        position(n + ip + 1, 1) = - (i2 - 0.5) * grid + offset;
        position(n + ip + 1, 2) = (i1 - 0.5) * grid;
...
A = [[diag(Zii * ones(n, 1)) zeros(n, n) (-ones(n, 1)) zeros(n, 1)]; [zeros(n, n) diag(Zii * ones(n, 1)) zeros(n, 1) (-ones(n, 1))]; [ones(1, n) zeros(1, n) 0 0]; [zeros(1, n) ones(1, n) 0 0]];
